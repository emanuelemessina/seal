...
min_size = max_size = (
    256  # so that the images are not resized! (and so the boxes aren't either)
)

backbone = resnet_fpn_backbone(
    backbone_name="resnet50", trainable_layers=5, returned_layers=[2, 3], weights=None
)
...
anchor_generator = AnchorGenerator(
    sizes=(
        ...
        (50,),
        (70,),
    ),  # we wouldn't want to use the pooling layer but can't see an option to disable it
    aspect_ratios=(
        (
            1.0,
            2.0,
...
rpn_head = RPNHead(
    backbone.out_channels, anchor_generator.num_anchors_per_location()[0]
)
...
featmap_names = [
    "0",
    "1",
]  # we use all of the returned feature maps, in this case we have just 2
roi_sampling_ratio = 2

multiscale_roi_align = MultiScaleRoIAlign(
    featmap_names=featmap_names,
    output_size=roi_output_size,
    sampling_ratio=roi_sampling_ratio,
)
box_roi_align = CustomRoIAlign(
    featmap_names=featmap_names,
    output_size=roi_output_size,
    sampling_ratio=roi_sampling_ratio,
)
...
class CustomPredictor(nn.Module):
    def __init__(
        ...
        self.box_distancer = nn.Sequential(
            nn.Linear(in_lin_features, 4096), nn.ReLU(), nn.Linear(4096, funneled_dim)
        )
        ...
        self.subclassifier_junction = nn.Sequential(
            nn.Conv2d(
                subcl_junc_chans, subcl_junc_chans, kernel_size=1
            ),  # dense like 1x1 + super
            nn.ReLU(),
        ...
        self.sub_classifiers = nn.ModuleList(
            [
                nn.Linear(2048, superclasses_groups[i][1])
                for i in range(len(superclasses_groups))
            ]
        )
